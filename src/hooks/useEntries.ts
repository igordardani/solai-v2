...
export function useEntries(userId: string | null, investmentValue: number) {
  ...
  // Carregar entradas e seed se vazio
  useEffect(() => {
    if (!userId) { setLoading(false); return; }

    const seedIfEmpty = async () => {
      const snap = await getDocs(query(collection(db, "users", userId, "entries"), limit(1)));
      if (!snap.empty) return;
      for (let i = 0; i < SEED_DATA.length; i += 5) {
        await Promise.all(
          SEED_DATA.slice(i, i + 5).map(([year, month, val]) =>
            addDoc(collection(db, "users", userId, "entries"), {
              year, month, discountValue: val, userId, createdAt: serverTimestamp(),
            })
          )
        );
      }
  ...
  // Métricas derivadas — recalculadas só quando entries ou investimento mudam
